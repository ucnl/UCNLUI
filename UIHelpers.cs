...
namespace UCNLUI
{
    public static class UIHelpers
    {
        ...
        public static void InvokeSetCheckedState(ToolStrip strip, ToolStripMenuItem item, bool checkedState)
        {
            if (strip.InvokeRequired)
                strip.Invoke((MethodInvoker)delegate { item.Checked = checkedState; });
            else
        ...
        public static void TrySetCbxItem(ToolStripComboBox cbx, string item)
        {
            int idx = cbx.Items.IndexOf(item);
            if (idx >= 0)
                cbx.SelectedIndex = idx;
